// ========== 五运六气计算引擎 · 云函数 ==========
// 迁移自吾乡帖 js/wuyunliuqi.js，去除 DOM 依赖
// 支持 action: 'current' | 'byDate' | 'analyze'
...
function detectClimateAlerts(daily) {
  ...
  for (let i = 1; i < tMax.length; i++) {
    const drop24h = tMax[i-1] - tMax[i];
    const rise24h = tMax[i] - tMax[i-1];

    if (drop24h >= 8) {
      alerts.push({ date: days[i], type:'寒潮降温', evil:'寒', msg: `${days[i].slice(5)} 最高温骤降 ${drop24h.toFixed(0)}°C，寒邪来袭`, priority: 'high' });
    } else if (rise24h >= 8) {
      alerts.push({ date: days[i], type:'热浪升温', evil:'热', msg: `${days[i].slice(5)} 最高温骤升 ${rise24h.toFixed(0)}°C，暑热突袭`, priority: 'high' });
    }
    if (i >= 2 && tMax[i] >= 35 && tMax[i-1] >= 35 && tMax[i-2] >= 35) {
      alerts.push({ date: days[i], type:'持续高温', evil:'暑', msg: `${days[i-2].slice(5)}起连续高温 ≥35°C`, priority: 'high' });
    }
    if (i >= 2 && precip[i] > 0 && precip[i-1] > 0 && precip[i-2] > 0) {
      alerts.push({ date: days[i], type:'持续阴雨', evil:'湿', msg: `${days[i-2].slice(5)}起连续3天有雨`, priority: 'medium' });
    }
  }
  ...
  for (let i = 2; i < tMax.length; i++) {
    ...
    if (Math.abs(range2 - range1) >= 10) {
      alerts.push({ date: days[i], type:'温差异常', evil:'风', msg: `${days[i].slice(5)} 昼夜温差剧烈变化`, priority: 'medium' });
    }
  }
...
function mergeAlerts(raw) {
  ...
  const ranges = [], singles = [], used = new Set();
  for (let i = 0; i < raw.length; i++) {
    ...
    if (a.type === '持续阴雨' || a.type === '持续高温') {
      ...
      ranges.push({ type: a.type, evil: a.evil, priority: a.priority, start: raw[i].date, end: raw[end].date, msg: a.msg });
    } else {
      singles.push(a);
    }
  }
  return [...ranges, ...singles].slice(0, 6);
}

// ========== 云函数入口 ==========
exports.main = async (event, context) => {
  const { action, date, weather, climateZone, cityInfo } = event;

  try {
    if (action === 'analyze') {
      // 运气 vs 天气 对照分析
      const wylq = getCurrentWYLQ(date ? new Date(date) : new Date());
      const result = analyzeQiVsWeather(wylq, weather, climateZone);
    ...
    if (action === 'guidance') {
      // 生成大白话指南
      const wylq = getCurrentWYLQ(date ? new Date(date) : new Date());
      const guidance = getPlainGuidance(wylq, weather, cityInfo);
      const weatherDiag = weather ? diagnoseWeather(weather) : null;
      return { success: true, data: { wylq, guidance, weatherDiag } };
    }

    if (action === 'alerts') {
      // 天气剧变检测
      const { daily } = event;
      const alerts = detectClimateAlerts(daily);
      return { success: true, data: alerts };
    }

    // 默认：返回当前五运六气
    const wylq = getCurrentWYLQ(date ? new Date(date) : new Date());
    return { success: true, data: wylq };
  } catch (err) {
